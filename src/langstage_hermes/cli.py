"""``langstage-hermes`` CLI — subcommands + chat REPL + slash-command dispatch.

SPEC §16 reproduction. Built on ``click`` for the same hyphenated subcommand
grammar Hermes ships (``langstage-hermes chat``, ``... cron list``, ...).
...
The ``chat`` command imports the agent module lazily — if ``langstage_hermes.agent``
...
def _print_banner(*, tagline: str | None = None) -> None:
    ...
    Called from the chat REPL on startup and from the bare ``langstage-hermes``
    invocation (above the help text). Skipped silently if stdout isn't a TTY
    so piped invocations stay clean (``langstage-hermes --version | grep``,
    test harnesses, etc.).
    """
    try:
...
def _instantiate_factory(target: Any, cfg: Any) -> Any:
    ...
    loaded via LANGSTAGE_AGENT_SPEC may not accept a config arg — they
...
def _try_import_agent() -> tuple[Any | None, str | None]:
    ...
    try:
        agent_mod = importlib.import_module("langstage_hermes.agent")
    ...
    if factory is None:
        return None, (
            "langstage_hermes.agent loaded but exposes neither create_hermes_agent "
...
@click.option(
    ...
    "agent_spec",
    default=None,
    help="Agent spec (module:attr or path/to/file.py:attr); overrides LANGSTAGE_AGENT_SPEC (or legacy DEEPAGENT_AGENT_SPEC).",
)
@click.option(
    "--workspace",
    "workspace",
...
def chat(model_id: str | None, agent_spec: str | None, workspace: str | None) -> None:
    ...
    session_id = getattr(agent, "langstage_hermes_session_id", None) or f"sess-{uuid.uuid4().hex[:12]}"
...
def _slash_tools(args: str, state: dict[str, Any]) -> bool:
    ...
    click.echo(
        click.style(
            f"  (+{stubbed} declared but stubbed — run `langstage-hermes tools` for the full taxonomy)",
            fg="bright_black",
        )
    )
    return False
...
def _slash_toolsets(args: str, state: dict[str, Any]) -> bool:
    ...
    cfg = state["cfg"]
    from langstage_hermes.tools.toolsets import IMPLEMENTED_TOOLSETS, resolve_enabled
...
def _slash_skills(args: str, state: dict[str, Any]) -> bool:
    ...
    if parts and parts[0] == "show" and len(parts) == 2:
        ...
        if len(skill.body) > 1500:
            ...
            click.echo(
                click.style(
                    f"\n... (+{extra} more chars — full body via `langstage-hermes skills show {skill.name}`)",
                    fg="bright_black",
                )
            )
        return False
...
def _slash_cron(args: str, state: dict[str, Any]) -> bool:
    ...
    items = list_jobs()
    if not items:
        click.echo("No cron jobs scheduled. Add one with `langstage-hermes cron create --prompt ... --schedule ...`.")
        return False
...
def audit_diff_cmd(skill_name: str, mutation_id: int) -> None:
    ...
    from langstage_hermes.skills.audit import RollbackError
...
def cron_daemon() -> None:
    """Run the cron daemon forever (alias for ``python -m langstage_hermes.cron``)."""
    from langstage_hermes.cron.__main__ import main as cron_main
...
def _curator_state_get() -> dict[str, Any]:
    from langstage_hermes.curator import _load_curator_state

    return _load_curator_state(_curator_store())


def _curator_state_save(state: dict[str, Any]) -> None:
    from langstage_hermes.curator import _save_curator_state
...
def plugins_enable(name: str) -> None:
    ...
    click.echo(f"(Plugin enable for {name!r} TBD — edit langstage-hermes.toml.)")
...
def verify(model_id: str | None, keep_workspace: bool) -> None:
    ...
    # try/finally so it's removed on EVERY exit path — build failure, invoke
    # failure, and the VERIFY: PASS success path alike (gh #68). `--keep-workspace`
    # is the opt-out for anyone who wants to poke at it post-mortem.
    ...
    try:
        ...
        click.echo(
            click.style(
                "  next: `langstage-hermes chat`  (or set LANGSTAGE_AGENT_SPEC=langstage_hermes.agent:graph in a host)",
                fg="bright_black",
            )
        )
    finally:
        if keep_workspace:
            click.echo(click.style(f"  · workspace kept at {workspace} for inspection", fg="bright_black"))
        else:
...
def main() -> None:
    """Console-script entry point (``langstage-hermes`` in pyproject)."""
