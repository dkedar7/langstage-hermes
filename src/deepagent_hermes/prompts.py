...
* **context**  — caller-supplied ``system_message`` plus context-file content
  ...
  :func:`deepagent_hermes.memory.threat_patterns.scan`; matches are replaced
...
:class:`deepagent_hermes.caching.AnthropicCachingS3Middleware`). All we do is
...
from deepagent_hermes.memory.threat_patterns import scan as _threat_scan
...
def build_context_files_prompt(cwd: Path | None = None) -> str:
    ...
    :func:`deepagent_hermes.memory.threat_patterns.scan` with ``scope="context"``.
