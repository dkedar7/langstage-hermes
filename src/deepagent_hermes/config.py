"""`HermesConfig` — full TOML/env-resolved configuration for ``deepagent-hermes``.
...
Resolution chain (lowest to highest precedence):
    ...
    defaults  <  ~/.deepagent-hermes/config.toml  <  ./deepagent-hermes.toml
...
keys live only in ``deepagent-hermes.toml``.
...
HERMES_GLOBAL_TOML = Path.home() / ".deepagent-hermes" / "config.toml"
HERMES_PROJECT_TOML = "deepagent-hermes.toml"


def hermes_home() -> Path:
    """Resolve ``HERMES_HOME`` with the documented precedence.

    Order: ``DEEPAGENT_HERMES_HOME`` env > ``HERMES_HOME`` env > default
...
def _hermes_global_toml_path() -> Path:
    """Compute the global Hermes config path, honoring ``DEEPAGENT_HERMES_HOME``."""
    base = hermes_home()
    return base / "config.toml"


def _find_hermes_project_toml(start: Path | None = None) -> Path | None:
    """Walk up from ``start`` (or cwd) looking for ``deepagent-hermes.toml``."""
    here = (start or Path.cwd()).resolve()
    for directory in (here, *here.parents):
        candidate = directory / HERMES_PROJECT_TOML
...
def load_hermes_toml_config(start: Path | None = None) -> tuple[dict, list[Path]]:
    """Load + deep-merge the global and project ``deepagent-hermes.toml`` files.
...
class HermesConfig(HostConfig):
    """Full ``deepagent-hermes`` runtime config.
    ...
    # ── TOML key map (dotted path within deepagent-hermes.toml) ──
    ...
    def resolve(  # type: ignore[override]
        ...
        *,
        ...
        Layers (lowest precedence first):
          ...
          3. ``~/.deepagent-hermes/config.toml`` then ``./deepagent-hermes.toml``
