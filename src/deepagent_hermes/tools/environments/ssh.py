...
class SshEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        *,
        ...
        if timeout is not None:
            ...
            env_to = os.environ.get("DEEPAGENT_HERMES_SSH_TIMEOUT")
    ...
    def _snapshot_path(self) -> PurePosixPath:  # type: ignore[override]
        ...
        other side.
        """
        return PurePosixPath(f"/tmp/deepagent-hermes-snap-{self.session_id}.sh")

    def _cwd_path(self) -> PurePosixPath:  # type: ignore[override]
        """Remote CWD marker — POSIX path in remote ``/tmp``."""
        return PurePosixPath(f"/tmp/deepagent-hermes-cwd-{self.session_id}.txt")
