...
Discovery order, lowest-to-highest precedence (later wins on name collision):
  ...
  2. **user**        — ``~/.langstage-hermes/plugins/<name>/``
  3. **project**     — ``./.langstage-hermes/plugins/<name>/``
                        (opt-in: ``DEEPAGENT_HERMES_ENABLE_PROJECT_PLUGINS=1``)
  4. **entry_point** — ``importlib.metadata.entry_points(group="langstage_hermes.plugins")``
...
from langstage_hermes.plugins.context import LoadedPlugin, PluginContext

logger = logging.getLogger(__name__)

ENTRY_POINTS_GROUP = "langstage_hermes.plugins"
...
class HermesPluginLoader:
    ...
    ``[plugins.disabled]`` from ``langstage-hermes.toml``. ``enabled=None``
    ...
    def _project_dir(self) -> Path:
        """Return ``./.langstage-hermes/plugins`` (or the test override)."""
        if self._project_dir_override is not None:
            return self._project_dir_override
        return Path.cwd() / ".langstage-hermes" / "plugins"

    @staticmethod
    def _project_plugins_enabled() -> bool:
        """``True`` iff ``DEEPAGENT_HERMES_ENABLE_PROJECT_PLUGINS`` is truthy."""
        val = os.getenv("LANGSTAGE_HERMES_ENABLE_PROJECT_PLUGINS") or os.getenv("DEEPAGENT_HERMES_ENABLE_PROJECT_PLUGINS", "")
    ...
    def _load_register_fn(plugin: LoadedPlugin) -> Any:
        ...
        # ``sys.modules``.
        mod_name = f"langstage_hermes_plugin_{plugin.source}_{plugin.name}"
