"""Cron daemon (60s tick) + ``tick()`` + ``run_job()`` for ``deepagent-hermes``.

SPEC §14 reproduction. Single long-running process (``python -m
deepagent_hermes.cron``) sweeps ``<HERMES_HOME>/cron/jobs.json`` every
...
def _build_cron_response(job: dict[str, Any], *, prompt: str) -> str:
    ...
    try:
        ...
        return f"[deepagent-hermes.agent unavailable; prompt was] {prompt}"
